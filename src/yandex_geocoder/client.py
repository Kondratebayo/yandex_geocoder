...
from yandex_geocoder.models.domain import GeocodeResult
...
class AsyncYandexGeocoderClient:
    ...
    def __init__(
        ...
        *,
        ...
        self._check_type(headers, (Mapping, ), "headers")
        ...
        self.timeout = (
            timeout if timeout is not None else httpx2.Timeout(DEFAULT_TIMEOUT)
        )
        self._default_params = (
            default_params if default_params is not None 
            else BaseGeocoderParams(geocode=None)
        )
        self.headers = headers if headers is not None else {}

        self._owns_client = http_client is None
        self._client = (
            http_client if http_client is not None else httpx2.AsyncClient()
        )
    ...
    def _make_status_error(self, exc: httpx2.HTTPStatusError) -> Exception:
        ...
        match status_code:
            case 400:
                return exceptions.BadRequestError(error_message)
            case 401:
                return exceptions.UnauthorizedError(error_message)
            case 403:
                return exceptions.ForbiddenError(error_message)
            case 429:
                return exceptions.RateLimitError(error_message)
            case status_code if 500 <= status_code < 600:
                return exceptions.ServerError(error_message)
            case _:
                # Fallback
                return exceptions.RequestError(f"HTTP {status_code}: {error_message}")
        ...
        *,
        ...
        if params is not None:
            ...
            try:
                ...
                raise exceptions.InvalidParamsError(message) from exc

        query = request.to_query_params()

        return query | {"apikey": self.__token.get_secret_value()}

    def _prepare_request(self, params: Mapping[str, Any]) -> httpx2.Request:
        return httpx2.Request(
            ...
            params=httpx2.QueryParams(params)
        )
    ...
    async def _request(
        ...
        try:
            ...
            raise exceptions.RequestTimeoutError(
                "Timeout occurred while requesting"
            ) from exc
        except httpx2.RequestError as exc:
            raise exceptions.RequestError(
                "Network/transport error occurred while requesting"
            ) from exc
    ...
    async def _execute(
        ...
        *,
        ...
        request = self._prepare_request(params=query_params)
        raw_data = await self._request(request)

        try:
            dto = GeocodeResponse.model_validate(raw_data)
            return dto.to_domain()
        except ValidationError as exc:
            message = handle_query_exception(exc)
            raise exceptions.UnexpectedResponseError(message) from exc
    ...
    async def geocode(
        ...
        **kwargs: Unpack[ForwardKwargs],
        ...
        return await self._execute(
            ...
            geocode=geocode,
            params=params,
            kwargs=kwargs, # pyright: ignore[reportArgumentType]
        )
